...
use aes_gcm::{Aes256Gcm, Key, Nonce as AesNonce, KeyInit};
...
impl FileEncryptor {
    ...
    pub fn encrypt_with_nonce(&self, data: &[u8], nonce: &Nonce) -> Result<EncryptedData> {
        ...
        let ciphertext = self.cipher.encrypt(aes_nonce, data)
...
impl FileDecryptor {
    ...
    pub fn decrypt(&self, encrypted_data: &EncryptedData) -> Result<Vec<u8>> {
        ...
        let plaintext = self.cipher.decrypt(aes_nonce, encrypted_data.ciphertext.as_slice())
            .map_err(|_| CarbideError::Internal("Decryption failed - invalid ciphertext or key".to_string()))?;
...
impl KeyDerivation {
    ...
    pub fn derive_from_password(password: &str, salt: &[u8], iterations: u32) -> Result<EncryptionKey> {
        if password.is_empty() {
            return Err(CarbideError::Internal("Password cannot be empty".to_string()));
        }
        
        if salt.len() < 16 {
            return Err(CarbideError::Internal("Salt must be at least 16 bytes".to_string()));
        }
    ...
    pub fn derive_from_master_key(master_key: &EncryptionKey, context: &[u8]) -> Result<EncryptionKey> {
...
impl KeyManager {
    ...
    pub fn import_encrypted_master_key(encrypted_master: &EncryptedMasterKey, password: &str) -> Result<Self> {
        let password_key = KeyDerivation::derive_from_password(password, &encrypted_master.salt, encrypted_master.iterations)?;
        
        let decryptor = FileDecryptor::new(&password_key)?;
        let master_key_bytes = decryptor.decrypt(&encrypted_master.encrypted_key)?;
        
        if master_key_bytes.len() != 32 {
            return Err(CarbideError::Internal("Invalid master key length".to_string()));
        }
...
mod tests {
    ...
    fn test_encrypted_master_key() {
        ...
        let encrypted_master = original_manager.export_encrypted_master_key(password).unwrap();
        
        // Import and verify
        let restored_manager = KeyManager::import_encrypted_master_key(&encrypted_master, password).unwrap();

...
use blake3::Hasher;
use carbide_core::{ContentHash, FileChunk, CarbideError, Result};
    ...
    pub fn new(chunk_size: u64) -> Result<Self> {
        if chunk_size < MIN_CHUNK_SIZE || chunk_size > MAX_CHUNK_SIZE {
            return Err(CarbideError::Internal(
                format!("Chunk size must be between {}MB and {}MB", 
    ...
    pub fn reassemble_chunks(chunks: &[FileChunk]) -> Result<Vec<u8>> {
        ...
        for chunk in &sorted_chunks {
            if chunk.offset != expected_offset {
                return Err(CarbideError::Internal(
                    format!("Missing chunk at offset {}", expected_offset)
                ));
            }
        ...
        if expected_offset != total_size {
            return Err(CarbideError::Internal(
                format!("Chunks total {} bytes but expected {}", expected_offset, total_size)
            ));
        }
    ...
    pub fn verify_and_reassemble(&self, chunks: &[FileChunk]) -> Result<Vec<u8>> {
        ...
        for chunk in chunks {
            if !self.verify_chunk(chunk) {
                return Err(CarbideError::Internal(
                    format!("Chunk at offset {} failed integrity check", chunk.offset)
                ));
            }
        }
...
impl MerkleTreeBuilder {
    ...
    pub fn build_tree(chunk_hashes: &[ContentHash]) -> Result<MerkleTree> {
        if chunk_hashes.is_empty() {
            return Err(CarbideError::Internal("Cannot build tree from empty chunks".to_string()));
        }
...
impl MerkleTree {
    ...
    pub fn get_proof(&self, chunk_index: usize) -> Result<MerkleProof> {
        ...
        if chunk_index >= leaf_count {
            return Err(CarbideError::Internal(
                format!("Chunk index {} out of bounds (max {})", chunk_index, leaf_count - 1)
            ));
        }

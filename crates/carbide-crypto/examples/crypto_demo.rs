...
fn demo_file_chunking() -> carbide_core::Result<()> {
    ...
    println!("\n📄 Large file ({:.1} MB):", large_file_data.len() as f64 / (1024.0 * 1024.0));
    ...
    println!("   Integrity check: ✅ {} bytes reassembled correctly", reassembled.len());
    
    // Show chunk details
    println!("\n   Chunk details:");
    for (i, chunk) in large_processed.chunks.iter().take(3).enumerate() {
        println!("     Chunk {}: {} bytes at offset {}, hash: {}...",
            i, chunk.data.len(), chunk.offset, 
            &chunk.hash.to_hex()[..16]);
    }
    if large_processed.chunks.len() > 3 {
        println!("     ... and {} more chunks", large_processed.chunks.len() - 3);
    }
...
fn demo_encryption() -> carbide_core::Result<()> {
    ...
    println!("📄 Original data ({} bytes): \"{}\"", 
        test_data.len(), String::from_utf8_lossy(test_data));
    ...
    println!("   Encrypted size: {} bytes (includes 16-byte auth tag)", encrypted_data.ciphertext.len());
    println!("   Nonce: {}", encrypted_data.nonce.to_hex());
    println!("   Ciphertext: {}...", hex::encode(&encrypted_data.ciphertext[..16]));
    ...
    println!("   ✅ Same file always gets same key: {}", 
        file1_key.as_bytes() == file1_key_again.as_bytes());
    println!("   ✅ Different files get different keys: {}", 
    ...
    let restored_manager = KeyManager::import_encrypted_master_key(&encrypted_master, export_password)?;
...
fn demo_merkle_trees() -> carbide_core::Result<()> {
    ...
    let chunk_hashes: Vec<ContentHash> = chunk_data.iter()
    ...
    println!("   Efficiency: {:.1}x smaller", 
...
fn demo_proof_of_storage() -> carbide_core::Result<()> {
    ...
    println!("   Expected response: {}...", &challenge.expected_response_hash.to_hex()[..16]);

...
use crate::{ReputationAlert, ReputationEvent};
...
impl ReputationStorage for MemoryStorage {
    ...
    async fn get_events_since(
        ...
        match events.get(provider_id) {
            ...
            None => Err(CarbideError::NotFound(format!(
                "No events found for provider {provider_id}"
            ))),
        }
    }
    ...
    async fn get_reputation(&self, provider_id: &ProviderId) -> Result<ReputationScore> {
        ...
        match scores.get(provider_id) {
            ...
            None => Err(CarbideError::NotFound(format!(
                "No reputation found for provider {provider_id}"
            ))),
        }
    }
    ...
    async fn get_top_providers(&self, limit: usize) -> Result<Vec<(ProviderId, ReputationScore)>> {
        ...
        let mut provider_scores: Vec<_> = scores
            .iter()
    ...
    async fn cleanup_old_alerts(&mut self, cutoff: DateTime<Utc>) -> Result<u64> {
        ...
        alerts.retain(|_, alert| alert.active || alert.triggered_at >= cutoff);
    ...
    async fn get_statistics(&self) -> Result<StorageStatistics> {
        ...
        let active_alerts = alerts.values().filter(|alert| alert.active).count() as u64;
...
impl FileStorage {
    ...
    pub async fn new<P: AsRef<Path>>(data_dir: P) -> Result<Self> {
        ...
        std::fs::create_dir_all(&data_dir)
            .map_err(|e| CarbideError::Internal(format!("Failed to create data directory: {e}")))?;
    ...
    async fn load_from_disk(&mut self) -> Result<()> {
        ...
        if events_file.exists() {
            let data = tokio::fs::read_to_string(&events_file)
                .await
        ...
        if scores_file.exists() {
            let data = tokio::fs::read_to_string(&scores_file)
                .await
        ...
        if alerts_file.exists() {
            let data = tokio::fs::read_to_string(&alerts_file)
                .await
    ...
    async fn save_to_disk(&self) -> Result<()> {
        ...
        let events_file = self.data_dir.join("events.json");
        tokio::fs::write(&events_file, &events_data)
            .await
        ...
        let scores_file = self.data_dir.join("scores.json");
        tokio::fs::write(&scores_file, &scores_data)
            .await
        ...
        let alerts_file = self.data_dir.join("alerts.json");
        tokio::fs::write(&alerts_file, &alerts_data)
            .await
...
mod tests {
    ...
    async fn test_memory_storage_reputation() {
        ...
        storage
            .store_reputation(&provider_id, &score)
            .await
            .unwrap();

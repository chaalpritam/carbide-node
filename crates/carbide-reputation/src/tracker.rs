...
use crate::{
    ReputationConfig, ReputationEvent, ReputationSystem, ReputationStatistics, 
    ReputationAlert, ReputationStorage, AlertType, AlertSeverity,
...
impl ReputationTracker {
    ...
    pub async fn process_events_batch(&mut self, events: Vec<ReputationEvent>) -> Result<Vec<ReputationUpdate>> {
    ...
    async fn calculate_reputation_score(&self, provider_id: &ProviderId) -> Result<ReputationScore> {
        let cutoff_date = Utc::now() - chrono::Duration::days(self.config.max_event_age_days as i64);
        let events = self.storage.get_events_since(provider_id, cutoff_date).await?;
    ...
    async fn generate_statistics(&self, provider_id: &ProviderId) -> Result<ReputationStatistics> {
        ...
        let average_uptime = if !uptime_events.is_empty() {
            let online_time: i64 = uptime_events.iter()
        ...
        let response_times: Vec<u64> = events.iter()
        ...
        let contract_violations = events.iter()
            .filter(|e| matches!(e.event_type, crate::events::EventType::ContractViolated { .. }))
            .count() as u64;

        // Analyze trend
        let historical_scores: Vec<Decimal> = all_events.iter()
            .rev()
        ...
        let first_event = all_events.first().map(|e| e.timestamp).unwrap_or_else(Utc::now);
    ...
    async fn check_for_alerts(&mut self, updates: &[ReputationUpdate]) -> Result<()> {
        for update in updates {
            ...
            if let Some(prev_score) = &update.previous_score {
                ...
                if score_drop > Decimal::new(2, 1) { // 0.2 drop
                    new_alerts.push(ReputationAlert {
                        ...
                        message: format!("Reputation dropped by {:.1}%", score_drop * Decimal::new(100, 0)),
            ...
            if stats.proof_success_rate < Decimal::new(80, 2) { // Below 80%
                new_alerts.push(ReputationAlert {
                    id: uuid::Uuid::new_v4(),
                    provider_id: update.provider_id,
            ...
            if stats.average_uptime < Decimal::new(95, 2) { // Below 95%
                new_alerts.push(ReputationAlert {
                    id: uuid::Uuid::new_v4(),
                    provider_id: update.provider_id,
    ...
    pub async fn perform_maintenance(&mut self) -> Result<u64> {
        ...
        let cutoff_date = Utc::now() - chrono::Duration::days(self.config.max_event_age_days as i64 * 2);
...
impl ReputationSystem for ReputationTracker {
    ...
    fn get_top_providers(&self, limit: usize) -> Result<Vec<(ProviderId, ReputationScore)>> {
        ...
        runtime.block_on(async {
            self.storage.get_top_providers(limit).await
        })
    }
...
mod tests {
    use super::*;
    use crate::{MemoryStorage, EventType, EventSeverity, events::EventBuilder};

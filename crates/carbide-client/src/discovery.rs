...
use carbide_core::{
    Provider, ProviderId, Region, ProviderTier, Result, CarbideError,
    network::*,
};
...
impl DiscoveryClient {
    ...
    pub async fn get_marketplace_stats(&self) -> Result<MarketplaceStatistics> {
        ...
        if !response.status().is_success() {
            return Err(CarbideError::Discovery(
                format!("Stats request returned: {}", response.status())
            ));
        }
        ...
        let stats = MarketplaceStatistics {
            ...
            available_capacity_gb: raw_stats["available_capacity_bytes"].as_u64().unwrap_or(0) as f64 
                / (1024.0 * 1024.0 * 1024.0),
            average_price_per_gb: raw_stats["average_price_per_gb"].as_str()
                .unwrap_or("0")
                .parse()
            ...
            last_updated: raw_stats["last_updated"].as_str()
                .and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok())
                .map(|dt: chrono::DateTime<chrono::FixedOffset>| dt.with_timezone(&chrono::Utc))
                .unwrap_or_else(chrono::Utc::now),
        };

        info!("Marketplace stats: {} providers, {:.1} GB available", 
    ...
    pub async fn search_providers(&self, query: MarketplaceQuery) -> Result<Vec<MarketplaceProvider>> {
        ...
        if !response.status().is_success() {
            return Err(CarbideError::Discovery(
                format!("Provider search returned: {}", response.status())
            ));
        }
        ...
        let marketplace_providers: Vec<MarketplaceProvider> = provider_list.providers
            .into_iter()
        ...
        info!("Found {} providers matching query", marketplace_providers.len());

        Ok(marketplace_providers)
    }

    /// Search providers with advanced filtering
    pub async fn search_providers_advanced(&self, filter: ProviderFilter) -> Result<Vec<MarketplaceProvider>> {
        ...
        let filtered_providers: Vec<MarketplaceProvider> = all_providers
            ...
            .filter(|mp| {
                ...
                if !filter.regions.is_empty() {
                    if !filter.regions.contains(&mp.provider.region) {
                        return false;
                    }
                }
                ...
                if !filter.tiers.is_empty() {
                    if !filter.tiers.contains(&mp.provider.tier) {
                        return false;
                    }
                }
    ...
    pub async fn compare_quotes(&self, request: &StorageQuoteRequest) -> Result<QuoteComparison> {
        debug!("Getting quotes for {}GB file, {} replicas, {} months", 
        ...
        if providers.is_empty() {
            return Err(CarbideError::Internal("No providers found for quote request".to_string()));
        }
        ...
        for marketplace_provider in providers {
            ...
            match self.client.request_storage_quote(provider_endpoint, request).await {
                Ok(quote_response) => {
                    // Calculate a score for this quote (lower is better)
                    let price_score = quote_response.price_per_gb_month.to_f32().unwrap_or(1.0);
                    let reputation_score = 1.0 - marketplace_provider.provider.reputation.overall.to_f32().unwrap_or(0.5);
                    ...
                    let combined_score = price_score * 0.5 + reputation_score * 0.3 + capacity_score * 0.2;
                ...
                Err(e) => {
                    debug!("Failed to get quote from {}: {}", 
        ...
        if quotes.is_empty() {
            return Err(CarbideError::Internal("No quotes received from any provider".to_string()));
        }

        // Sort quotes by score (best first)
        quotes.sort_by(|a, b| a.score.partial_cmp(&b.score).unwrap_or(std::cmp::Ordering::Equal));

        // Find best quotes
        let best_price_quote = quotes.iter()
            .min_by(|a, b| a.quote.price_per_gb_month.cmp(&b.quote.price_per_gb_month))
            .cloned();

        let best_reputation_quote = quotes.iter()
            .max_by(|a, b| a.provider.reputation.overall.cmp(&b.provider.reputation.overall))
            .cloned();

        // Calculate average price
        let total_price: rust_decimal::Decimal = quotes.iter()
            .map(|q| q.quote.price_per_gb_month)
            .sum();
        ...
        info!("Quote comparison: {} quotes, best price: ${}/GB/month", 
              comparison.quotes.len(),
              comparison.best_price_quote.as_ref()
                  .map(|q| q.quote.price_per_gb_month.to_string())
    ...
    pub async fn get_provider_details(&self, provider_id: &ProviderId) -> Result<MarketplaceProvider> {
        ...
        if !response.status().is_success() {
            return Err(CarbideError::Discovery(
                format!("Provider lookup returned: {}", response.status())
            ));
        }

        let registry_entry: serde_json::Value = response
            .json::<serde_json::Value>()
            .await
            .map_err(|e| CarbideError::Discovery(format!("Failed to parse provider details: {}", e)))?;
        ...
        let marketplace_provider = MarketplaceProvider {
            ...
            last_seen: registry_entry["last_heartbeat"].as_str()
                .and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok())
                .map(|dt: chrono::DateTime<chrono::FixedOffset>| dt.with_timezone(&chrono::Utc))
                .unwrap_or_else(chrono::Utc::now),
            provider,
    ...
    pub async fn register_provider(&self, provider: &Provider) -> Result<()> {
        ...
        if !response.status().is_success() {
            return Err(CarbideError::Discovery(
                format!("Provider registration returned: {}", response.status())
            ));
        }
    ...
    pub async fn health_check(&self) -> Result<HealthCheckResponse> {
        ...
        if !response.status().is_success() {
            return Err(CarbideError::Discovery(
                format!("Discovery health check returned: {}", response.status())
            ));
        }
        ...
        let health: HealthCheckResponse = response
            ...
            .await
            .map_err(|e| CarbideError::Discovery(format!("Failed to parse health response: {}", e)))?;

...
use carbide_client::{
    CarbideClient, ClientConfig, StorageManager, StoragePreferences, 
    DiscoveryClient, MarketplaceQuery, ProviderFilter, simple,
    ProgressCallback, StorageProgress,
};
use carbide_core::{ContentHash, Region, ProviderTier, ProviderRequirements};
use std::sync::Arc;
...
async fn demo_discovery_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match discovery.get_marketplace_stats().await {
        Ok(stats) => {
            ...
            println!("   Available Capacity: {:.2} GB", stats.available_capacity_gb);
    ...
    match discovery.search_providers(query).await {
        Ok(providers) => {
            println!("   Found {} Professional providers in North America:", providers.len());
    ...
    match discovery.search_providers_advanced(filter).await {
        Ok(providers) => {
            println!("   Found {} high-end providers in Europe/Asia:", providers.len());
            for provider in providers {
                println!("      • {} ({:?}, {:?}) - Capacity: {:.1}TB", 
                        provider.provider.name,
                        provider.provider.tier,
...
async fn demo_storage_management() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let storage_manager = StorageManager::with_preferences(
        client,
    ...
    match storage_manager.store_file(test_data, 6, Some(progress_callback)).await {
        Ok(store_result) => {
            println!("   ✅ File stored successfully!");
            println!("      Replicated to: {} providers", store_result.providers.len());
            println!("      Total Cost: ${}/month", store_result.total_monthly_cost);
    ...
    match storage_manager.retrieve_file(&file_id, access_token, Some(progress_callback)).await {
        Ok(retrieve_result) => {
...
async fn demo_simple_apis() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match simple::store_file(test_data, 3).await {
        Ok(result) => {
            println!("   ✅ Stored with {} replicas, cost: ${}/month", 

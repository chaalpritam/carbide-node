...
fn main() {
    ...
    println!("  Small file: {} ({} bytes, ID: {})", 
        small_file.name, small_file.size, small_file.id);
    println!("  Large file: {} ({} bytes, ID: {}, needs chunking: {})", 
        large_file.name, large_file.size, large_file.id, large_file.needs_chunking());
    println!();
    ...
    for provider in &providers {
        println!("  {} ({:?} tier, {:.3}$/GB/month, {}GB capacity, reputation: {:.1})",
            provider.name,
            provider.tier,
...
fn demonstrate_storage_scenarios(small_file: &File, large_file: &File, providers: &[Provider]) {
    ...
    println!("     Requirements: {} copies, max ${}/GB/month", 
    ...
    for provider in &suitable_providers {
        ...
        println!("       • {} (${}/month, {:.1}% reputation)", 
            provider.name, cost, provider.reputation.overall * Decimal::new(100, 0));
    }
    ...
    println!("     File: {} ({:.1} MB)", large_file.name, large_file.size as f64 / (1024.0 * 1024.0));
    println!("     Requirements: {} copies, max ${}/GB/month", 
    ...
    println!("\n     💵 Cost Analysis:");
    println!("       Critical file total budget: ${}/month", critical_budget);
    println!("       Backup file total budget: ${}/month", backup_budget);
    println!("       Total monthly storage cost: ${}/month", critical_budget + backup_budget);
}

fn find_suitable_providers<'a>(request: &StorageRequest, providers: &'a [Provider]) -> Vec<&'a Provider> {
    providers.iter()
        .filter(|provider| {
            // Check if provider meets requirements
            let meets_uptime = provider.reputation.uptime >= request.requirements.min_uptime;
            let meets_reputation = provider.reputation.overall >= request.requirements.min_reputation;
            let within_budget = provider.price_per_gb_month <= request.max_price_per_gb_month;
            let home_provider_ok = !request.requirements.exclude_home_providers || 
                                 provider.tier != ProviderTier::Home;
...
fn demonstrate_marketplace_economics(providers: &[Provider]) {
    ...
    for provider in providers {
        let savings_vs_aws = (Decimal::new(23, 3) - provider.price_per_gb_month) / Decimal::new(23, 3) * Decimal::new(100, 0);
        println!("    {}: {:.0}% cheaper than AWS",
    ...
    println!("    • Total network capacity: {:.1} GB", 
        providers.iter().map(|p| p.total_capacity as f64 / (1024.0 * 1024.0 * 1024.0)).sum::<f64>());
    println!("    • Average reputation: {:.2}", 
        providers.iter().map(|p| p.reputation.overall).sum::<Decimal>() / Decimal::new(providers.len() as i64, 0));
    println!("    • Price range: ${:.3} - ${:.3}/GB/month", 
        providers.iter().map(|p| p.price_per_gb_month).min().unwrap(),
        providers.iter().map(|p| p.price_per_gb_month).max().unwrap());
}
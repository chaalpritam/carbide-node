...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server = ProviderServer::new(config, provider, storage_path, None, None, 60, Default::default(), Default::default(), None)?;
    ...
    if quote_response.status().is_success() {
        ...
        println!(
            "   Response: {}",
            serde_json::to_string_pretty(&quote_json)?
        );
    } else {
    ...
    println!(
        "     Content: {:?}",
        std::str::from_utf8(test_file_data).unwrap_or("<binary>")
    );
    ...
    let store_message = NetworkMessage::new(MessageType::StoreFileRequest(store_request));
        ...
        if let Some(message_data) = store_json.get("message_type").and_then(|mt| mt.get("data")) {
            if let (Some(upload_url), Some(upload_token)) = (
                ...
                if upload_response.status().is_success() {
                    ...
                    let retrieve_response = client.get(&retrieve_url).send().await?;
                    ...
                    if retrieve_response.status().is_success() {
                        ...
                        println!(
                            "   Response: {}",
                            serde_json::to_string_pretty(&retrieve_json)?
                        );

                        // Test file download
                        let download_url =
                            format!("{}/api/v1/download/{}", base_url, file_id.to_hex());
                        ...
                        if download_response.status().is_success() {
                            ...
                            if downloaded_data.as_ref() == test_file_data {
                                println!(
                                    "   🎯 Downloaded data matches original! Storage integrity \
                                     verified."
                                );
                            } else {
                    ...
                    let challenge_message =
                        NetworkMessage::new(MessageType::StorageChallenge(challenge.clone()));
